...
import { TTranslation, TTranslationModel, TValueTranslation, TMenuModel } from '@/types/types';
...
export default async function CreateTranslation(translationInput: TTranslationInput): Promise<{translation: TTranslation|null, userErrors: any}> {
    try {
        const {userId, projectId, menuId, subjectId, ...translationBody} = translationInput;
        ...
        const {errors: errorsDB, data: savedData} = await (async (data) => {
            try {
                ...
                const translation: TTranslationModel|null = await Translation.create({userId, projectId, menuId, subjectId, ...data.translation});
